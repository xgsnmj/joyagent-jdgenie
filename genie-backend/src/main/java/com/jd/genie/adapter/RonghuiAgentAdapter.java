...
public class RonghuiAgentAdapter implements AgentAdapter {
    ...
    public ChatResponse sendChatRequest(String sessionId,
        ...
        log.info("融汇适配器处理请求 - 会话ID: {}, 端点: {}, 外部会话ID: {}",
        ...
        new Thread(() -> {
            try {
                ...
                Request request = new Request.Builder()
                        .url(apiEndpoint)
                        .post(body)
                        .addHeader("Authorization", apiKey) // 认证方式需确认
                ...
                // 处理SSE流
                processRonghuiSSEStream(httpResponse.body().byteStream(), sessionId, emitter);
